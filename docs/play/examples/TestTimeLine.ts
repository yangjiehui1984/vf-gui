...
export default class TestTimeLine {
    ...
    private onLoad(app: vf.Application, uiStage: vf.gui.Stage) {
        ...
        const sheepline = new vf.gui.Timeline();
        //设置动画对象、时间、帧率
        ...
        sheepline.on(vf.gui.Interaction.ComponentEvent.COMPLETE, () => {
            console.log("COMPLETE");
        });

        sheepline.on(vf.gui.Interaction.ComponentEvent.LOOP, () => {
            console.log("LOOP");
        });
...
class Sheep {
    ...
    public shape = new vf.gui.Graphics();
